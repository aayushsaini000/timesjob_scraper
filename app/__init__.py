...
#-----------Date time Settings of chainguardians schedulers--------------
    ...
    Timesjobs_scheduler.add_job(Timesjobs, trigger='cron', day_of_week='mon-sun', hour=13,minute=30)
    Timesjobs_scheduler.start()

    accountjobs_scheduler = BackgroundScheduler()
    accountjobs_scheduler.add_job(accountjobs, trigger='cron', day_of_week='mon-sun', hour=13,minute=30)
    accountjobs_scheduler.start()

    HRjobs_scheduler = BackgroundScheduler()
    HRjobs_scheduler.add_job(HRjobs, trigger='cron', day_of_week='mon-sun', hour=13,minute=30)
    HRjobs_scheduler.start()

    Salesjobs_scheduler = BackgroundScheduler()
    Salesjobs_scheduler.add_job(Salesjobs, trigger='cron', day_of_week='mon-sun', hour=13,minute=30)
    Salesjobs_scheduler.start()

    marketingjobs_scheduler = BackgroundScheduler()
    marketingjobs_scheduler.add_job(marketingjobs, trigger='cron', day_of_week='mon-sun', hour=13,minute=30)
    marketingjobs_scheduler.start()

    customerjobs_scheduler = BackgroundScheduler()
    customerjobs_scheduler.add_job(customerjobs, trigger='cron', day_of_week='mon-sun', hour=13,minute=30)
    customerjobs_scheduler.start()

    legaljobs_scheduler = BackgroundScheduler()
    legaljobs_scheduler.add_job(legaljobs, trigger='cron', day_of_week='mon-sun', hour=13,minute=30)
    legaljobs_scheduler.start()

    Teachingjobs_scheduler = BackgroundScheduler()
    Teachingjobs_scheduler.add_job(Teachingjobs, trigger='cron', day_of_week='mon-sun', hour=13,minute=30)
